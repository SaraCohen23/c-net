...
internal class SaleImplementation : ISale
{
    ...
    public int Create(Sale item)
    {
        ...
        List<Sale> list = load();
        //List<Product> p = null;
        //using (StreamReader sr = new StreamReader(@"../xml/Products.xml"))
        //{
        //    p = serializer.Deserialize(sr) as List<Product>;
        //}
        //Product pro = p.FirstOrDefault(i => i.productId == item.saleProductId);
        //if (pro == null)
        //    throw new DalDoesNotExistException("SALE PRODUCT ID NOT FOUND");
        list.Add(sale);
        save(list);
